# FinSolve AI Assistant - Terraform Infrastructure
...
resource "aws_launch_template" "app" {
  ...
  user_data = base64encode(templatefile("${path.module}/user-data.sh", {
    ...
    database_url       = "postgresql://finsolve_admin:${var.db_password}@${aws_db_instance.main.endpoint}:5432/finsolve"
    redis_url          = "redis://${aws_elasticache_cluster.main.cache_nodes[0].address}:6379"
    aws_region         = var.aws_region
    s3_data_bucket     = aws_s3_bucket.app_data.bucket
  }))
...
resource "aws_lb_listener" "ui" {
  ...
  tags = {
    Name        = "${var.project_name}-ui-listener"
    Environment = var.environment
  }
}
...
resource "aws_autoscaling_group" "main" {
  ...
  target_group_arns   = [aws_lb_target_group.api.arn, aws_lb_target_group.ui.arn]
