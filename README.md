...
graph TB
    subgraph "Executive Dashboard"
        ED[Real-time Business Intelligence<br/>Executive KPI Monitoring]
        EC[Strategic Analytics<br/>Performance Insights]
    end

    subgraph "Enterprise Gateway"
        LB[Production Load Balancer<br/>SSL + Rate Limiting]
        API[FastAPI + LangServe<br/>Production API Gateway]
        AUTH[Enterprise Authentication<br/>JWT + RBAC]
    end

    subgraph "AI Orchestration Layer (Innovation Core)"
        LG[LangGraph Workflow Engine<br/>Enhanced with MCP]
        QC[Intelligent Query Classifier<br/>Structured vs Semantic]
        RT{Smart Router<br/>Optimization Engine}
    end

    subgraph "MCP Innovation Layer"
        MC[MCP Client Orchestra<br/>Intelligent Tool Router]
        HR[HR MCP Server<br/>4 Specialized Tools]
        FIN[Finance MCP Server<br/>4 Specialized Tools]  
        DOC[Document MCP Server<br/>4 Specialized Tools]
    end

    subgraph "Advanced RAG Engine"
        RAG[Multimodal RAG Engine<br/>Text + Numerical Fusion]
        VDB[ChromaDB Vector Store<br/>EuriAI Embeddings]
        CONF[Confidence Scoring<br/>95%+ Accuracy Engine]
    end

    subgraph "Business Intelligence"
        VIZ[AI-Powered Visualization<br/>Auto Chart Generation]
        DASH[Interactive Dashboards<br/>Real-time Analytics]
        RPT[Executive Reporting<br/>Automated Insights]
    end

    subgraph "Enterprise Infrastructure"
        DB[(PostgreSQL<br/>Enterprise DB)]
        CACHE[(Redis Cluster<br/>Performance Cache)]
        MON[CloudWatch<br/>Monitoring + Alerts]
    end
    ...
    LG --> QC
    QC --> RT
    RT -->|Structured Queries| MC
    RT -->|Document Queries| RAG
    RT -->|Hybrid Queries| MC
    RT -->|Hybrid Queries| RAG
    MC --> HR
    MC --> FIN
    MC --> DOC
    RAG --> VDB
    ...
    style LG fill:#FF6B35,stroke:#0D1B2A,stroke-width:3px,color:#0D1B2A
    style MC fill:#FF6B35,stroke:#0D1B2A,stroke-width:3px,color:#0D1B2A
    style RAG fill:#4CAF50,stroke:#0D1B2A,stroke-width:3px,color:#0D1B2A
    style VIZ fill:#FF9800,stroke:#0D1B2A,stroke-width:3px,color:#0D1B2A
    style ED fill:#00F5D4,stroke:#0D1B2A,stroke-width:3px,color:#0D1B2A
```
...
```mermaid
graph TD
    A[Executive Query] --> B[Enterprise Authentication]
    B --> C[Strategic Query Analysis]
    C --> D{Intelligent Classification}

    D -->|Structured Business Data| E[MCP Processing Excellence]
    D -->|Strategic Documents| F[Advanced RAG Processing]
    D -->|Executive Insights| G[Hybrid Intelligence Fusion]

    E --> E1[MCP Client Orchestra]
    E1 --> E2{Business Domain Router}
    E2 -->|HR Analytics| E3[HR Intelligence Server]
    E2 -->|Financial Metrics| E4[Finance Intelligence Server]
    E2 -->|Document Intelligence| E5[Document Intelligence Server]

    F --> F1[Semantic Vector Search]
    F1 --> F2[Context-Aware Retrieval]

    G --> E1
    G --> F1

    E3 --> H[Structured Business Results]
    E4 --> H
    E5 --> H
    F2 --> I[Strategic Context Results]

    H --> J[Executive Data Fusion Engine]
    I --> J

    J --> K[Business Intelligence Decision]
    K --> L{Executive Visualization?}

    L -->|Yes| M[Professional Chart Generation]
    L -->|No| N[Executive Summary Only]

    M --> O[Strategic Response Synthesis]
    N --> O

    O --> P[Confidence Assessment]
    P --> Q[Executive Delivery]
...
graph TB
    ...
    subgraph "Application Tier (Auto-Scaling)"
        ASG[Auto Scaling Group<br/>3-10 Instances]
        EC2A[EC2 Instance A<br/>FinSolve AI App]
        EC2B[EC2 Instance B<br/>FinSolve AI App]
        EC2C[EC2 Instance C<br/>FinSolve AI App]
    end

    subgraph "Data Tier (High Availability)"
        RDS[RDS PostgreSQL<br/>Multi-AZ Deployment]
        REDIS[ElastiCache Redis<br/>Cluster Mode]
        S3[S3 Buckets<br/>Data + Static Assets]
    end

    subgraph "AI/ML Services"
        SAGE[SageMaker<br/>Model Hosting]
        COMP[Comprehend<br/>NLP Services]
        TEXT[Textract<br/>Document Processing]
    end
    ...
    style ASG fill:#FF6B35,stroke:#0D1B2A,stroke-width:3px
    style RDS fill:#4CAF50,stroke:#0D1B2A,stroke-width:3px
    style CW fill:#FF9800,stroke:#0D1B2A,stroke-width:3px
```
